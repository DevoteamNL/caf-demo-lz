locals {
  landingzone = {
    ...
    lower = {
      ...
      client_id            = try(var.landingzone.tfstates["lower"].client_id, var.backend.client_id)
      client_secret        = try(var.landingzone.tfstates["lower"].client_secret, var.backend.client_secret)
    }
  }
}
